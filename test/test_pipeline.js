...
describe('executes pipelines', () => {
  it('can construct an empty pipeline', (done) => {
    ...
    assert.deepStrictEqual(pipeline.transforms, [],
      'Empty pipeline should not have transforms')
    assert.deepStrictEqual(pipeline.requires(), [],
  ...
  it('executes a three-transform pipeline with a report', (done) => {
    ...
    assert(env.results.has('keyword'),
      'Expected a result')
    assert.deepStrictEqual(env.results.get('keyword'), fixture.TABLE,
  ...
  it('logs execution', (done) => {
    ...
    assert.deepStrictEqual(env.log, expected,
  ...
  it('complains about multiple results with the same name', (done) => {
    ...
    pipeline.run(env)
    assert.deepStrictEqual(env.log[env.log.length - 1],
...
describe('saving empty dataframes', () => {
  it('saves a dataframe with no columns', (done) => {
    const env = new Env(INTERFACE)
    const name = new Transform.name('top')
    const saveAs = new Transform.saveAs('bottom')
    const pipeline = new Pipeline(name, saveAs)
    pipeline.run(env)
    done()
  })
})
